...
import { customerLedgerColumns } from './tables/documentColumns';
...
const CustomerLedgerEntries = () => {
    ...
    const fetchCustomerLedgerEntries = async () => {
        ...
        setError("");
        try {
            const ledgers = await documentService.fetchCustomerLedgerEntries();
            setData(ledgers);
        } catch (err) {
            setError("Fetching Customer Ledger Entries failed. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-background text-foreground">
            ...
            <main id="dashboard" className="mx-auto max-w-[1120px] px-5 pb-10 pt-4 lg:px-0 lg:pt-10">
                <div className="mb-4 flex items-end justify-between gap-4">
                    <p className="mb-1 text-sm font-medium text-muted-foreground">Customer Ledger Entries</p>
                </div>
                {
                    isLoading ? (
                        ...
                        <div className="flex min-h-[400px] flex-col items-center justify-center gap-4 rounded-lg border border-red-200 bg-red-50 p-8 text-center">
                            ...
                            <button
                                onClick={() => fetchCustomerLedgerEntries()}
