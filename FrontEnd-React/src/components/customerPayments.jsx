...
import { customerPaymentColumns } from './tables/documentColumns';
...
const CustomerPayments = () => {
    ...
    const fetchCustomerPayments = async () => {
        ...
        setError("");
        try {
            const payments = await documentService.fetchCustomerPayments();
            setData(payments);
        } catch (err) {
            setError("Fetching Customer Payments failed. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-background text-foreground">
            ...
            <main id="dashboard" className="mx-auto max-w-[1120px] px-5 pb-10 pt-4 lg:px-0 lg:pt-10">
                <div className="mb-4 flex items-end justify-between gap-4">
                    <p className="mb-1 text-sm font-medium text-muted-foreground">Customer Payments</p>
                </div>
                {
                    isLoading ? (
                        ...
                        data && <DataTable data={data} columns={customerPaymentColumns} />
                    )
                }
            </main>
        </div>
